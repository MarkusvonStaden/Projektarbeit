...
if not client.collection_exists("hybrid_question_only"):
    client.create_collection(
        collection_name="hybrid_question_only",
        vectors_config={
            "mxbai-embed-large-v1": models.VectorParams(
                size=1024,
                distance=models.Distance.COSINE,
            ),
            "colbertv2.0": models.VectorParams(  # Matryoshka ColBERTv2
                size=128,
                distance=models.Distance.COSINE,
...
def search(query: str, threshold: float = 15):
    dense_query_vector = next(dense_embedding_model.embed(query)) # Use next()
    sparse_query_vector = next(bm25_embedding_model.embed(query)) # Use next() here as well
    late_query_vector = next(late_interaction_embedding_model.embed(query)) # And here

    prefetch = [
        models.Prefetch(
            query=dense_query_vector,
            using="mxbai-embed-large-v1",
            limit=20,
        ),
        models.Prefetch(
            # The sparse_query_vector is now the actual embedding object, not a generator
    ...
    results = client.query_points(
        "hybrid_question_only", # Assuming you fixed the collection name from the previous question
        prefetch=prefetch,
...
if __name__ == "__main__":
    ...
    questions = [data["frage"] for data in test_data]
    answers = [data["antwort"] for data in test_data]
    dense_embeddings = list(dense_embedding_model.embed(questions))
    bm25_embeddings = list(bm25_embedding_model.embed(questions))
    late_interaction_embeddings = list(late_interaction_embedding_model.embed(questions))
    ...
    client.upload_points(
        ...
        points = [
            models.PointStruct(
                id=i,
                vector={
                    "mxbai-embed-large-v1": dense_embeddings[i].tolist(),
    ...
    search_results = (search("Wie verbinde ich meinen Mac mit dem WLAN?"), search("Wer ist Angelo Merte?"))
    scores_0 = [point.score for point in search_results[0].points]
    scores_1 = [point.score for point in search_results[1].points]
    pprint(f"Scores for query 1: {scores_0}")
    pprint(f"Scores for query 2: {scores_1}") 
    print(len(search_results[0].points))
    print(len(search_results[1].points))
    pprint(f"Search Results: {search_results[0]}")